...
import code.LoggerPeerInfo;
import code.PeerServer;
import code.RemotePeerDetails;
import code.ChokeScheduler;
import code.OptimisticUnchokeScheduler;

public class PeerMain {
	private String peerID;
	private RemotePeerDetails myConfig;
	private HashMap<String, RemotePeerDetails> peerInfoMap;
	private ArrayList<String> peerList;
	private volatile HashMap<String, PeerProcessHandler> joinedPeers;
	private volatile HashMap<String, Thread> joinedThreads;
	private volatile ServerSocket listener;
	private PeerServer server;
	private CommonConfig commonConfig;
	private PeerInfoConfig peerInfoConfig;
	private volatile LoggerPeerInfo logger;
	...
	private volatile ChokeScheduler chHandler;
	private volatile OptimisticUnchokeScheduler optHandler;
	private volatile TerminationImpl terminateHandler;
	private volatile HashMap<String, Integer> downloadRate;
	private Thread serverThread;
	private volatile Boolean iamDone;

	public PeerMain(String peerID) {
		this.peerID = peerID;
		...
		this.logger = new LoggerPeerInfo(this.peerID);
		this.iamDone = false;
		this.unChokedList = new HashSet<>();
		this.interestedList = new HashSet<>();
		this.init();
		this.chHandler = new ChokeScheduler(this);
		this.downloadRate = new HashMap<>();
		this.optHandler = new OptimisticUnchokeScheduler(this);
		this.terminateHandler = new TerminationImpl(this);
		this.chHandler.startJob();
		this.optHandler.startTask();
	}

	public void init() {
		try {
			this.commonConfig.loadCommonFile();
			this.peerInfoConfig.loadCfgFile();
			...
			this.peerInfoMap = this.peerInfoConfig.getPeerMap();
			...
			this.checkPieceIfAvailable();
			this.startServer();
			this.createConnectionsNeigh();
		} 
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void startServer() {
		try {
			this.listener = new ServerSocket(this.myConfig.pPort);
	...
	public void createConnectionsNeigh() {
		try {
			Thread.sleep(5000);
			for (String pid : this.peerList) {
				if (pid.equals(this.peerID)) {
					...
					RemotePeerDetails peer = this.peerInfoMap.get(pid);
					Socket temp = new Socket(peer.pAddress, peer.pPort);
					PeerProcessHandler p = new PeerProcessHandler(temp, this);
					p.setEndPeerID(pid);
					this.initiJoinedPeer(p, pid);
	...
	public synchronized void broadcastIfHave(int pieceIndex) {
		for (String key : this.joinedPeers.keySet()) {
	...
	public synchronized void initiJoinedPeer(PeerProcessHandler p, String endpeerid) {
	...
	public PeerProcessHandler getPeerHandler(String peerid) {
		return this.joinedPeers.get(peerid);
	}

	public BitSet getIfAvailable(String pid) {
		return this.piecesAvailability.get(pid);
	}

	public synchronized boolean checkInterest(String endpeerid) {
		BitSet end = this.getIfAvailable(endpeerid);
		BitSet mine = this.getIfAvailable(this.peerID);
	...
	public LoggerPeerInfo getLogger() {
		return this.logger;
	}

	public boolean hasFile() {
		return this.myConfig.cFile == 1;
	}

	public int getNoOfPreferredNeighbors() {
		return this.commonConfig.noOfPreferredNeighbors;
	}
	...
	public String getFileName() {
		return this.commonConfig.fName;
	}

	public int getFileSize() {
		return this.commonConfig.fSize;
	}

	public int getPieceSize() {
		return this.commonConfig.pSize;
	}
	...
	public synchronized OptimisticUnchokeScheduler getoptHandler() {
		return this.optHandler;
	}

	public synchronized ChokeScheduler getchHandler() {
	...
	public synchronized void cancelChokes() {
		try {
			...
			this.iamDone = true;
			this.terminateHandler.startTask(6);
		} 
		catch (Exception e) {
			e.printStackTrace();
		}
